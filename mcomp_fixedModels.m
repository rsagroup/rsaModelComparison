...
switch (what)
    ...
    case 'modelCompare'
        ...
        %         rsa_testModelCompare('modelCompare','model','START_compl.mat','numSim',100,'outfile','sim_pcm_Exp3.mat','methods',{'loglikPCM'},'Omega',[0:0.1:0.8]);
        %
    ...
    case 'encodeReg'
        ...
        % rsa_testModelCompare('encodeReg','model','Model_fiveFinger.mat','numSim',1000,'outfile','sim_encodeReg_Exp1.mat','Omega',0.1);
        % rsa_testModelCompare('encodeReg','model','Model_fiveFinger.mat','numSim',50,'method','encodePCM','outfile','sim_encodePCM_Exp1.mat','Omega',0.3);
        ...
        % rsa_testModelCompare('encodeReg','model','Model_fiveFinger.mat','numSim',50,'method','encodeGCV','outfile','sim_encodeGCV_Exp1.mat');
                ...
                for n=1:size(T.RDM,1)
                    ...
                    for v=Opt.numReg
                        switch(Opt.method)
                            ...
                            case 'encodePCM'
                                [T.(sprintf('R2_%2.2d',v))(n,m),~,h,T.(sprintf('corr_%2.2d',v))(n,m)]= ...
                                    encode_crossval(Y(:,:,n),Xcond*X{m}(:,1:v),part,'pcm_NR_diag','X',Xpart,'Gd',ones(v,1));
                                T.(sprintf('lambda_%2.2d',v))(n,m)=mean(h(:,2)./h(:,1)); % Sigma_eps / Sigma_a
                                T.(sprintf('lambStd_%2.2d',v))(n,m)=std(h(:,2)./h(:,1)); % Sigma_eps / Sigma_a
                            case 'encodeGCV' % Generalized crossvalidaton 
                                [T.(sprintf('R2_%2.2d',v))(n,m),~,h,T.(sprintf('corr_%2.2d',v))(n,m)]= ...
                                    encode_crossval(Y(:,:,n),Xcond*X{m}(:,1:v),part,'GCV','X',Xpart,'Gd',ones(v,1));
                                T.(sprintf('lambda_%2.2d',v))(n,m)=mean(h);
                                T.(sprintf('lambStd_%2.2d',v))(n,m)=std(h);
                            case 'encodeRidge'
                                [T.(sprintf('R2_%2.2d',v))(n,m),~,T.(sprintf('lambda_%2.2d',v))(n,m),T.(sprintf('corr_%2.2d',v))(n,m)]= ...
    ...
    case 'Figure_numReg' % This is the encoding model figure for different numbers of regressors
        cd(baseDir);
        methods={'encodeGCV','encodePCM'};   %,'encodeRidge'
        indx = {1,[2 3],[4 5]};
        ...
        for ex=1
            ...
            lineplot(D.numReg,[D.propCorrR2],'split',D.method,'style_thickline','leg',methods);
            hold on;
    ...
    case 'Figure_regularisation'
        ...
        wysiwyg
    case 'Figure_bestMethods'
        methods={'loglikIRLS','loglikPCM','encodePCMcorr','encodePCM'};
        methType = [1 2 3 4]';
        evalType = [1 1 1 1]';
        numMethods = length(methods);
        cd (baseDir); 
        for ex=1:3
            ...
            subplot(2,3,ex+3);
            barplot(D.methType,D.avrgTime,'subset',D.methType~=3);
